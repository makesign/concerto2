...
class User < ActiveRecord::Base
  ...
  has_many :leading_groups, lambda {
  ...
  def owned_feeds
    ...
    (leading_groups + supporting_groups(:feed, %i[all submissions])).collect(&:feeds).flatten.sort.uniq
  end

  # Return an array of all the groups a user has a certain regular permission for.
  def supporting_groups(type, permissions)
    groups.select { |g| g.user_has_permissions?(self, :regular, type, permissions) }
  end
end
