...
class Content < ApplicationRecord
  ...
  default_scope { where(type: Content.all_subclasses.collect(&:name)) }
  ...
  def is_approved?
    approved_feeds.count.positive? && (pending_feeds.count + denied_feeds.count).zero?
  end

  # Determine if content is pending on a feed
  def is_pending?
    pending_feeds.count.positive?
  end

  # Determine if content is denied on a feed
  def is_denied?
    denied_feeds.count.positive?
  end
  ...
  def self.form_attributes
    [:name, :duration, :data, { start_time: %i[time date] }, { end_time: %i[time date] }]
  end
  ...
  def self.all_subclasses
    sub = []
    sub.concat(subclasses)
    subclasses.each do |subklass|
  ...
  def self.display_name
    if const_defined?('DISPLAY_NAME') && !self::DISPLAY_NAME.nil?
      self::DISPLAY_NAME
    else
      model_name.human
    end
  ...
  def action_allowed?(_action_name, _user)
    false
  end
  ...
  def perform_action(action_name, options)
    return unless action_allowed?(action_name, options[:current_user])
  ...
  def after_add_callback(unused_submission); end
  ...
  def can_display_in?(_screen, field)
    kind == field.kind
  end
  ...
  def self.filter_all_content(params)
    ...
    if params[:screen].present?
      ...
      screen_feeds.each do |feed|
        ...
        filtered_contents += if params[:is_active].present?
                               feed.contents.where(content_params).active
                             # Allow expired and future contents
                             else
                               feed.contents.where(content_params)
                             end
      end
