...
class MediaController < ApplicationController
  ...
  def create
    ...
    files.each do |file|
      ...
      if media.file_size.positive? && Concerto::ContentConverter.supported_types.include?(media.file_type)
  ...
  def get_file_params(h = params.permit!.to_hash)
    ...
    h.each do |k, v|
      ...
      if value.is_a?(Hash) || value.is_a?(Array)
        ...
        files << k if k.is_a?(ActionDispatch::Http::UploadedFile)
        files << v if v.is_a?(ActionDispatch::Http::UploadedFile)
      end
    end
    files
  end
