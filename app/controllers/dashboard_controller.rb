...
class DashboardController < ApplicationController
  ...
  def show
    if current_user
      ...
      @concerto_plugins = can?(:read, ConcertoPlugin) ? ConcertoPlugin : nil
  ...
  def get_activities(activity_limit)
    ...
    if current_user && defined?(PublicActivity::Activity)
      if current_user.is_admin?
        ...
        owner = PublicActivity::Activity.where(owner_id: current_user.id,
                                               owner_type: 'User').order('updated_at desc').limit(activity_limit)
        recipient = PublicActivity::Activity.where(recipient_id: current_user.id,
                                                   recipient_type: 'User').order('updated_at desc').limit(activity_limit)

        # Select the activities that involve a group as the recipient for which the user is a member
        group_member = PublicActivity::Activity.where(recipient_id: current_user.group_ids,
                                                      recipient_type: 'Group').order('updated_at desc').limit(activity_limit)
        ...
        public_activities = PublicActivity::Activity.where(owner_id: nil,
                                                           recipient_id: nil).where(PublicActivity::Activity.arel_table[:trackable_type].not_eq('ConcertoConfig')).order('updated_at desc').limit(activity_limit)
