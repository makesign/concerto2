...
module ConcertoConfigHelper
  def generate_tabs(configs)
    ...
    configs.distinct.each_with_index do |c, i|
      if c.category != last_category
        headers << "<li#{class_attribute if i.zero?}><a href=\"#tab#{c.id}\" data-toggle=\"tab\">#{c.category}</a></li>"
  ...
  def rails_tmp_text
    "#{Rails.root.join('tmp')} #{@rails_tmp_perms ? 'is' : 'should be'} writable by the webserver."
  end
