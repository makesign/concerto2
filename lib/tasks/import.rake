      ...
      ActiveRecord::Base.transaction do
        ...
        if !content_type.nil?
          ...
          if content_type == Graphic
            ...
            new_content.media.build(key: 'original', file:, file_type: c.mime_type)
          end
          new_content.save
  ...
  task submissions: :environment do
    ...
    V1Submission.all.each do |s|
      ActiveRecord::Base.transaction do
        ...
        unless feeds.include?(s.feed_id)
          # puts "Feed missing."
          next
        end
        if s.moderator_id.zero? || (!s.moderation_flag.nil? && s.moderator_id.nil?)
    ...
    ActiveRecord::Base.transaction do
      V1Template.all.each do |t|
        ...
        t.fields.each do |f|
          ...
          new_field = Field.where(name: f.name.split.first).first if new_field.nil?
          new_field = Field.where(name: f.name.split.last.pluralize).first if new_field.nil?
        ...
        file = File.new(filename, 'r')
        new_template.media.build(key: 'original', file:, file_type: 'image/jpeg')
      ...
      V1Screen.all.each do |s|
        s.template.fields.each do |v1_field|
          ...
          positions.each do |p|
            ...
            unless position_mapping.include?(p.field_id)
              puts 'Unable to find position for this subscription.  Skipping'
              next
            end
            unless feeds.include?(p.feed_id)
