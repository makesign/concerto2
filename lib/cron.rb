...
module Clockwork
  ...
  if RUBY_VERSION >= '1.9'
    every(1.day, 'Remove old public activity entries') do
      unless ConcertoConfig[:keep_activity_log].to_i.zero?
        PublicActivity::Activity.where('created_at < :days',
                                       { days: ConcertoConfig[:keep_activity_log].to_i.days.ago }).destroy_all
      end
    end
  end
