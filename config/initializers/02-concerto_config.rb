...
Rails.configuration.after_initialize do
  if ActiveRecord::Base.connection.data_source_exists?('concerto_configs') && ConcertoConfig.columns_hash.key?('plugin_id')
    ...
    ConcertoConfig.make_concerto_config('content_default_start_time', '12:00 am', value_type: 'time',
                                                                                  category: 'Content', seq_no: 20)
    ConcertoConfig.make_concerto_config('content_default_end_time', '11:59 pm', value_type: 'time',
                                                                                category: 'Content', seq_no: 30)
    ...
    # access
    ConcertoConfig.make_concerto_config('public_concerto', 'true', value_type: 'boolean', category: 'Permissions',
                                                                   seq_no: 99)
    ConcertoConfig.make_concerto_config('allow_registration', 'true', value_type: 'boolean', category: 'Permissions',
                                                                      seq_no: 1)
    ConcertoConfig.make_concerto_config('confirmable', 'false', value_type: 'boolean', category: 'Permissions',
                                                                seq_no: 2)
    ConcertoConfig.make_concerto_config('allow_user_screen_creation', 'false', value_type: 'boolean',
                                                                               category: 'Permissions', seq_no: 99)
    ConcertoConfig.make_concerto_config('allow_user_feed_creation', 'true', value_type: 'boolean',
                                                                            category: 'Permissions', seq_no: 99)

    # mail
    ConcertoConfig.make_concerto_config('mailer_protocol', 'sendmail', value_type: 'select',
                                                                       select_values: "{'Sendmail' => 'sendmail', 'SMTP' => 'smtp'}", category: 'Mail', seq_no: 1)
    ConcertoConfig.make_concerto_config('mailer_from', 'concerto@localhost', value_type: 'string',
                                                                             value_default: 'concerto@localhost', category: 'Mail', seq_no: 2)
    ...
    ConcertoConfig.make_concerto_config('worker_heartbeat', '0', value_type: 'integer', category: 'Processing',
                                                                 hidden: 'true', can_cache: false)
    ...
    ConcertoConfig.make_concerto_config('secret_token', '', value_type: 'string', value_default: '', hidden: 'false',
                                                            category: 'System')
  end
end
